...
\begin{abstract}
  ...
  different modalities and backbones and never co-evaluated under one protocol. Reading
  them together, we find three things: the shaping rules succeed only on the
  architecture they were designed on, with the data-optimized Optimal-FS and the
  conservative ReAct transferring while the aggressive pruning rules collapse; the
  cluster unifies only partially, as whole-vector ASH and the Jacobian-based BLOOD lie
  outside the optimization that explains the element-wise rules; and BLOOD, used as a
  control, shows the OOD signal is not carried by the feature values alone. A distance
  baseline, Mahalanobis, bounds both settings from above.
\end{abstract}
...
practitioners increasingly rely on large pre-trained models they did not train and
cannot easily retrain, using them as frozen checkpoints. This makes \emph{post-hoc}
OOD detection---methods that attach to an already-trained network at inference time,
add no training cost, and leave its weights untouched---the dominant practical setting,
and the one this report studies.

Within this setting, \emph{feature-based} detectors read the penultimate-layer
representation---the feature vector a network produces just before its final linear
classifier---rather than the output logits alone. Most form a tight family this report
calls the \emph{shaping cluster}: ReAct~\citep{react} (rectified activations),
ASH~\citep{ash} (activation shaping), VRA~\citep{vra} (variational rectified
activation), and Optimal-FS~\citep{optimalfs}\footnote{Our shorthand for the otherwise
unnamed method of \citet{optimalfs}, after the paper's title \emph{Towards Optimal
Feature-Shaping Methods}.} each insert a fixed transformation that rewrites the feature
...
classifier with a penultimate layer---so on paper none is restricted to, or excluded
from, any model family.
...
vector and can be recovered by reshaping it---and BLOOD, by reading a property of the
transformation rather than editing the vector, serves as a control on that assumption.
Because the methods were developed on different modalities and backbones and never
co-evaluated under a single protocol, the report compares them with that
incomparability in mind, asking three questions in turn: why the same method succeeds
...
post-hoc---the lightweight setting that dominates practice.

Post-hoc detectors differ in which model-internal quantity they read: the logits
(e.g.\ MSP, MaxLogit, energy), a feature-space distance (e.g.\ Mahalanobis), a
...
The first branch---\emph{feature shaping}---replaces $\mathbf{z}$ with $f(\mathbf{z})$
before the logits are recomputed, with $f$ chosen to sharpen the ID/OOD gap while
leaving ID accuracy intact; ReAct~\cite{react}, ASH~\cite{ash}, VRA~\cite{vra}, and
...
a single ID-only optimum. BLOOD~\cite{blood} sits in the second branch: it leaves
...
validation set, or training intervention. This axis cuts across the cluster:
...
shaping functions to ID data and are open-box---so the sharp line is not BLOOD
against the shaping cluster but the data-free methods (ASH, BLOOD) against the
ID-data-dependent ones. The strongest open-box reference point, the Mahalanobis
distance detector~\cite{mahalanobis}---which fits a class-conditional Gaussian to ID
features and scores by distance to the nearest class mean---belongs to neither
paradigm; it appears only as a benchmark ceiling in the Optimal-FS and BLOOD
experiments, to which we defer it.
...
placing them in the open-box category (cf.\ Section~\ref{subsec:access}). The amplification of mid-range activations
...
common optimum. Dropping the OOD term, which contributes little beyond the ID
direction, gives a closed-form solution from ID data alone (the optimization problem
...
absorbing the most training-driven change. The advantage is more consistent on ELECTRA than on RoBERTa, with BLOOD$_L$
significantly improving over the MSP baseline on six of the eight ELECTRA
datasets~\citep{blood}. Notably, the two shaping methods that appear here, ASH and
ReAct, are only middling, indicating that vision-tuned shaping does not transfer to
text. BLOOD's grouping confirms our access axis (Section~\ref{subsec:access}): its Table~1
splits the baselines into white/black-box and open-box columns, filing ASH as
white-box but ReAct as \emph{open-box}---explicitly because ReAct's clip uses the
$90$th percentile of the \emph{training-data} activations, the same data-dependence
criterion our axis applies. BLOOD's open-box set is therefore
ReAct, ENSM (ensemble), TEMP (temperature scaling), and Mahalanobis, so the
data-free/data-dependent line we draw
coincides with BLOOD's rather than cutting more strictly than it.
...
satisfy. The islands therefore deliver a structural message, not a numerical ranking:
each paradigm wins only within its own setting, and a simple distance detector tops both
...
(rectified linear unit) convolutional premise---whereas BLOOD reads transformation
smoothness in the upper layers of a transformer, where fine-tuning
concentrates~\citep{blood}. Each method encodes the architecture it was designed on,
which is why neither transfers.
...
best in a setting-free sense; each encodes the architecture and modality it was born
on, and a simple distance baseline outperforms both paradigms wherever its assumptions
hold. We leave open whether the whole-vector case admits the same optimization account
...
\end{equation}
the isotropy of the probes being what makes the estimator unbiased.
